...
// ----- настройки радуги
float RAINBOW_STEP = 3.50;         // шаг изменения цвета радуги
...
float SMOOTH = 0.05;               // коэффициент плавности анимации VU (по умолчанию 0.5)
#define MAX_COEF 1.8              // коэффициент громкости (максимальное равно среднему * этот коэф) (по умолчанию 1.8)

// ----- режим цветомузыки
float SMOOTH_FREQ = 0.3;          // коэффициент плавности анимации частот (по умолчанию 0.8)
float MAX_COEF_FREQ = 1.2;        // коэффициент порога для "вспышки" цветомузыки (по умолчанию 1.5)
uint16_t SMOOTH_STEP = 5;            // шаг уменьшения яркости в режиме цветомузыки (чем больше, тем быстрее гаснет)
...
int RAINBOW_PERIOD = 10;
float RAINBOW_STEP_2 = 2;
...
void animation() {
  ...
  switch (this_mode) {
    ...
    case 4: //радуга-громкость
      ...
      break;
    case 5: //светомузыка
        for (int i = 0; i < NUM_LEDS; i++) leds[i] = CHSV(LIGHT_COLOR, LIGHT_SAT, thisBright[0]);
      break;
    case 6: //бегущие частоты
       for (int i = 0; i < NUM_LEDS; i++) leds[i] = CHSV(LIGHT_COLOR, LIGHT_SAT, thisBright[1]);
      break;
    case 7: //стробоскоп-реакция
      for (int i = 0; i < NUM_LEDS; i++) leds[i] = CHSV(LIGHT_COLOR, LIGHT_SAT, thisBright[2]);
      break;
    case 8: //огонь
      fireRoutine(false);
      delay(50);
...
void remoteTick() {
  ...
  if (ir_flag) { // если данные пришли
    ...
    switch (IRdata) {
      ...
      case BUTT_HASH:
        switch (this_mode) {
          case 5: if(SMOOTH_STEP == 21) {
            SMOOTH_STEP = 1;
          }
            SMOOTH_STEP = smartIncr(SMOOTH_STEP, 4, 1, 21);
            break;
          case 6: if(SMOOTH_STEP == 21) {
            SMOOTH_STEP = 1;
          }
            SMOOTH_STEP = smartIncr(SMOOTH_STEP, 4, 1, 21);
            break;
          case 7: if(SMOOTH_STEP == 21) {
            SMOOTH_STEP = 1;
          }
            SMOOTH_STEP = smartIncr(SMOOTH_STEP, 4, 1, 21);
            break;
        ...
        if (settings_mode) {
          ...
          switch (this_mode) {
            ...
            case 1: LIGHT_SAT = smartIncr(LIGHT_SAT, 20, 0, 255);
              break;
            case 5: LIGHT_SAT = smartIncr(LIGHT_SAT, 20, 0, 255);
              break;
            case 6: LIGHT_SAT = smartIncr(LIGHT_SAT, 20, 0, 255);
              break;
      ...
      case BUTT_LEFT:
        if (settings_mode) {
          ...
          BRIGHTNESS = smartIncr(BRIGHTNESS, -30, 0, 255);
          FastLED.setBrightness(BRIGHTNESS);
        } else {
          switch (this_mode) {
